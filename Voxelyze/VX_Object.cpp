...
//	growthModel = 0.0;
	...
	if (!pXML->FindLoadElement("numStiffnessModelSynapses", &numStiffnessModelSynapses)) numStiffnessModelSynapses = 0;
	...
	if (pXML->FindElement("StiffnessModelSynapseWeights")){
		int voxCounter = 0;
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					for (int s=0; s<numStiffnessModelSynapses; s++)
					{
						SetStiffnessModelSynapseWeight(voxCounter,s,atof(dataArray[numStiffnessModelSynapses*k+s].c_str()));
					}
					voxCounter++;
				}
			}
	...
	if (pXML->FindElement("DisintegrationOrder")){
		usingDisintegration = true;
		int voxCounter = 0;

		InitDisintegrationOrderArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetDisintegrationOrder(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	{
		usingDisintegration = false;
	}


	if (pXML->FindElement("Insulation")){
		usingInsulation = true;
		int voxCounter = 0;

		InitInsulationArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetInsulation(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	{
		usingInsulation = false;
	}


	if (pXML->FindElement("ComputationalBoundary")){
		usingComputationalBoundary = true;
		int voxCounter = 0;

		InitComputationalBoundaryArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetComputationalBoundary(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	{
		usingComputationalBoundary = false;
	}
	...
	{ 
		...
		pXML->FindLoadElement("MinElasticMod", &DeprecatedMinElasticMod);
        pXML->FindLoadElement("MaxElasticMod", &DeprecatedMinElasticMod);
