...
void CVXS_SimGLView::DrawFloor(void)
{
	...
	vfloat offset_Z=(pSim->LocalVXC.GetVZDim()*Size)/2;
	...
	vfloat pipe=pSim->pEnv->GetPipe()+Size/50;
	//pipe=pipe+0.00001;
	...
	for (int i=-80; i <=80; i++){
		for (int j=-80; j <=80; j++){
			...
			// PLAIN GRAY FLOOR
			glColor4d(0.21,0.21,0.21, 1.0);
			glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
			glBegin(GL_QUADS);
			vfloat floorHeight = bump_height*(sin(bump_seperation*(i*sX+Size))*cos(bump_seperation*(j*sY))/bump_seperation);
			glVertex3d(i*sX+Size, j*sY, floorHeight);
			floorHeight =bump_height*(sin(bump_seperation*(i*sX+Size))*cos(bump_seperation*(j*sY+Size))/bump_seperation);
			glVertex3d(i*sX+Size, j*sY+Size, floorHeight);
			floorHeight = bump_height*(sin(bump_seperation*(i*sX))*cos(bump_seperation*(j*sY+Size))/bump_seperation);
			glVertex3d(i*sX, j*sY+Size, floorHeight);
			floorHeight = bump_height*(sin(bump_seperation*(i*sX))*cos(bump_seperation*(j*sY))/bump_seperation);
			glVertex3d(i*sX, j*sY, floorHeight);

			glEnd();

			glColor4d(0.6, 0.7+0.2*((int)(1000*sin((float)(i+100)*(j+103)*(j+369)))%10)/10.0, 0.6, 1.0);
			if(pSim->fluidEnvironment){glColor4d(0.6,0.6,0,1);}
			
			glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );
			glBegin(GL_QUADS);
			floorHeight = bump_height*(sin(bump_seperation*(i*sX+Size))*cos(bump_seperation*(j*sY))/bump_seperation);
			glVertex3d(i*sX+Size, j*sY, floorHeight);
			floorHeight =bump_height*(sin(bump_seperation*(i*sX+Size))*cos(bump_seperation*(j*sY+Size))/bump_seperation);
			glVertex3d(i*sX+Size, j*sY+Size, floorHeight);
			floorHeight = bump_height*(sin(bump_seperation*(i*sX))*cos(bump_seperation*(j*sY+Size))/bump_seperation);
	...
	if(pSim->pEnv->IsPipeEnabled()){
		...
		vfloat sX = Size*0.0001;
		vfloat sY = Size;
		//Size=Size/2;
		vfloat time_b4_pipe=8;

		for (int i=-160000; i <=160000; i++){
			
			glColor4d(0.21,0.21,0.21, 1.0);

			glPolygonMode( GL_FRONT_AND_BACK, GL_FILL );
			glBegin(GL_QUADS);
			vfloat floorHeight =- sqrt(pipe-pow(i*sX-offset_X+Size,2))+offset_Y;
			glVertex3d(i*sX+Size,floorHeight,sY);
			floorHeight =-sqrt(pipe-pow(i*sX-offset_X+Size,2))+offset_Y;
			glVertex3d(i*sX+Size,floorHeight,sY*10);
			floorHeight = -sqrt(pipe-pow(i*sX-offset_X,2))+offset_Y;
			glVertex3d(i*sX, floorHeight, sY*10);
			floorHeight =-sqrt(pipe-pow(i*sX-offset_X,2))+offset_Y;
			glVertex3d(i*sX,floorHeight,sY);
...
CColor CVXS_SimGLView::GetCurVoxColor(int SIndex, int Selected)
{
	...
	switch (CurViewCol) {
		case RVC_TYPE:
		{
            if (pSim->VoxArray[SIndex].disintegrated)
                return CColor(0.0f, 0.0f, 0.0f, 0.5f);

		    else if (pSim->pEnv->getGrowthAmplitude() > 0)  // color difference from nominal
            {
            ...
            else if (pSim->pEnv->pObj->GetUsingComputationalBoundary())
                return GetJet(pSim->VoxArray[SIndex].ComputationalBoundary/0.1);
...
//            break;
		...
		case RVC_KINETIC_EN: // Quickly remapped on delta stiffness  --> electricity --> disintegration
...
//                return CColor(0.0f, 1.0f, 1.0f, 1.0f);
                 ...
                 if(pSim->pEnv->GetDisintegrationFromPressureThreshold()  or pSim->pEnv->GetDisintegrationFromStressThreshold()){
                    float DisintegrationPressure = 0;
                    float DisintegrationStress = 0;

                    if(pSim->pEnv->GetDisintegrationFromPressureThreshold())
                        DisintegrationPressure = pSim->VoxArray[SIndex].GetDisintegrationPressureIntegral() / pSim->pEnv->GetDisintegrationFromPressureThreshold();

                    if(pSim->pEnv->GetDisintegrationFromStressThreshold())
                        DisintegrationStress = pSim->VoxArray[SIndex].GetDisintegrationStressIntegral() / pSim->pEnv->GetDisintegrationFromStressThreshold();

                    if (DisintegrationPressure > DisintegrationStress)
                        return GetJet(DisintegrationPressure);
                    else
                        return GetJet(DisintegrationStress);
                 }

                 else
                 {
                    if (pSim->pEnv->pObj->GetDisintegrationOrder(SIndex) > pSim->pEnv->GetNumDisintegrationUpdates())
                        return GetJet(0);
                    else if (pSim->pEnv->pObj->GetDisintegrationOrder(SIndex) < pSim->NextVoxToRemove)
                        return GetJet(1 - (pSim->NextVoxToRemove - pSim->pEnv->pObj->GetDisintegrationOrder(SIndex)) / pSim->pEnv->GetNumDisintegrationUpdates());
                    else
                        return GetJet(pSim->pEnv->pObj->GetDisintegrationOrder(SIndex) / pSim->pEnv->GetNumDisintegrationUpdates());
                 }
...
//				return CColor(R, G, B, A);
//			}
            if (pSim->VoxArray[SIndex].Insulation)
                return CColor(1.0, 0.647, 0.0, 1.0);
            else
                return CColor(0.0, 0.0, 1.0, 0.1);
			break;
...
//			return GetJet(fabs(pSim->VoxArray[SIndex].GetMaxBondStress() / pSim->VoxArray[SIndex].PreDamageStress));
		...
		case RVC_DISP:
		{
			...
			break;
//			double maxS = pSim->pEnv->GetMaxAdaptationRate();
//			double minS = -pSim->pEnv->GetMaxAdaptationRate();
...
//			break;
		...
		case RVC_STRAIN_EN:  // Now: light intensity or pitch  --> electricity
		{
			//if (pSim->SS.MaxBondStrainE == 0) return GetJet(0);
			//return GetJet(pSim->VoxArray[SIndex].GetMaxBondStrainE() / pSim->SS.MaxBondStrainE);
			//break;

//			double maxP = pSim->pEnv->GetMaxAdaptationRate();
//			double minP = -pSim->pEnv->GetMaxAdaptationRate();
