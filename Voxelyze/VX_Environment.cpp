...
void CVX_Environment::WriteXML(CXML_Rip* pXML)
{
	pXML->DownLevel("Environment");
		...
		pXML->Element("PipeEnabled", PipeEnabled);
		pXML->Element("IceEnabled", IceEnabled);
		pXML->UpLevel();
...
bool CVX_Environment::ReadXML(CXML_Rip* pXML, std::string* RetMessage) //pXML pointer to an "Environment" element
{
	...
	if (pXML->FindElement("Gravity")){
		...
		if (!pXML->FindLoadElement("PipeEnabled", &PipeEnabled)) PipeEnabled = false;
		if (!pXML->FindLoadElement("IceEnabled", &IceEnabled)) IceEnabled = false;
		if (!pXML->FindLoadElement("FloorSlope", &FloorSlope)) FloorSlope = 0.0;
		if (!pXML->FindLoadElement("bump_size", &bump_size)) bump_size = 0.0;
		if (!pXML->FindLoadElement("pipe", &pipe)) pipe = 0.0;
		if (!pXML->FindLoadElement("bump_seperation", &bump_seperation)) bump_seperation = 0.0;
	...
	if (pXML->FindElement("RegenerationModel")){
		...
		if (!pXML->FindLoadElement("RegenerationModelTimeConstant", &RegenerationModelTimeConstant)) RegenerationModelTimeConstant = 1;
		pXML->UpLevel();
	}

	if (pXML->FindElement("StiffnessModel")){
	    if (!pXML->FindLoadElement("StiffnessModelUpdatesPerTempCycle", &StiffnessModelUpdatesPerTempCycle)) StiffnessModelUpdatesPerTempCycle = 0.0;
		if (!pXML->FindLoadElement("NumHiddenStiffnessNeurons", &NumHiddenStiffnessNeurons)) NumHiddenStiffnessNeurons = 2;
		if (!pXML->FindLoadElement("StiffnessModelInputBias", &StiffnessModelInputBias)) StiffnessModelInputBias = false;
		if (!pXML->FindLoadElement("StiffnessModelTimeConstant", &StiffnessModelTimeConstant)) StiffnessModelTimeConstant = 1;
		pXML->UpLevel();
	}
	...
	if (pXML->FindElement("Disintegration")){
	    if (!pXML->FindLoadElement("DisintegrationUpdatesPerTempCycle", &DisintegrationUpdatesPerTempCycle)) DisintegrationUpdatesPerTempCycle = 0.0;
	    if (!pXML->FindLoadElement("NumDisintegrationUpdates", &NumDisintegrationUpdates)) NumDisintegrationUpdates = 0;
	    if (!pXML->FindLoadElement("DisintegrationFromPressureThreshold", &DisintegrationFromPressureThreshold)) DisintegrationFromPressureThreshold = 0.0;
	    if (!pXML->FindLoadElement("DisintegrationFromStressThreshold", &DisintegrationFromStressThreshold)) DisintegrationFromStressThreshold = 0.0;
		pXML->UpLevel();
	}
    ...
    if (!pXML->FindLoadElement("GreedyStiffnessUpdatesPerTempCycle", &GreedyStiffnessUpdatesPerTempCycle)) GreedyStiffnessUpdatesPerTempCycle = 0.0;
	if (!pXML->FindLoadElement("GreedyStiffnessSpeedLimit", &GreedyStiffnessSpeedLimit)) GreedyStiffnessSpeedLimit = 0;
	if (!pXML->FindLoadElement("GreedyStiffness", &GreedyStiffness)) GreedyStiffness = false;
	if (!pXML->FindLoadElement("GreedyStiffnessThreshold", &GreedyStiffnessThreshold)) GreedyStiffnessThreshold = 0;

	if (!pXML->FindLoadElement("MinElasticMod", &MinElasticMod)) MinElasticMod = 1e7;
	if (!pXML->FindLoadElement("MaxElasticMod", &MaxElasticMod)) MaxElasticMod = 1e7;
	if (!pXML->FindLoadElement("MaxAdaptationRate", &MaxAdaptationRate)) MaxAdaptationRate = 0;
	if (!pXML->FindLoadElement("MaxStiffnessChange", &MAX_STIFFNESS_VARIATION_STEP)) MAX_STIFFNESS_VARIATION_STEP = 0;
