...
//		if (pXML->FindLoadElement("Integrator", &tmpInt)) CurIntegrator = (IntegrationType)tmpInt; else CurIntegrator = I_EULER;
	...
	if (pXML->FindElement("Collisions")){
	    if (!pXML->FindLoadElement("StickyVoxels", &StickyVoxels)) StickyVoxels = false;
...
int CVX_Sim::CreateStickyBond(int SIndexNegIn, int SIndexPosIn)
{
	CVXS_BondInternal tmp(this);
	if (!tmp.LinkVoxels(SIndexNegIn, SIndexPosIn)) return -1;
...
//	// make sure there is no voxel at that lattice point already
//	if (AXIS_X){
//	    if (VoxArray[SIndexNegIn].GetInternalBondIndex(0) != -1) {BondArrayInternal.pop_back(); return -1;}
//	    if (VoxArray[SIndexPosIn].GetInternalBondIndex(1) != -1) {BondArrayInternal.pop_back(); return -1;}
//    }
//    if (AXIS_Y){
//	    if (VoxArray[SIndexNegIn].GetInternalBondIndex(2) != -1) {BondArrayInternal.pop_back(); return -1;}
//	    if (VoxArray[SIndexPosIn].GetInternalBondIndex(3) != -1) {BondArrayInternal.pop_back(); return -1;}
//    }

    // todo: if voxels are rotated and connect weirdly they create constant torque moment
...
void CVX_Sim::DetachVox(int VoxIndex)
{
	// std::cout << "detaching vox " << VoxIndex << std::endl;

	// pacify the discarded voxel so it doesn't bob around on the floor
	VoxArray[VoxIndex].Pacify();

	// flag that the voxel was detached
	VoxArray[VoxIndex].disintegrated = true;

    // find the bonds to delete
	std::vector<int> trash;
	int iT = NumBond();
	for (int j=0; j<iT; j++) {
	    if (BondArrayInternal[j].GetVox1SInd() == VoxIndex) trash.push_back(j);
	    if (BondArrayInternal[j].GetVox2SInd() == VoxIndex) trash.push_back(j);
	}

    // nullify pointers to the removed bond
    CVXS_Voxel* pV1, *pV2;
	for(std::vector<int>::size_type j=trash.size(); j>0; j--) {

        // std::cout << "trash " << j-1 << ": "<< trash[j-1] << std::endl;
	    pV1 = BondArrayInternal[trash[j-1]].GetpV1(); pV2 = BondArrayInternal[trash[j-1]].GetpV2();

	    pV1->NeighborJustDetached = true;
	    pV2->NeighborJustDetached = true;

	    for (int k=0; k<6; k++) {
	        if(pV1->GetInternalBondIndex(k) == trash[j-1]){
	            pV1->RemoveInternalBondIndex(k);
	            pV1->NullifyInternalBondPointer(k);
	        }
	        if(pV2->GetInternalBondIndex(k) == trash[j-1]){
	            pV2->RemoveInternalBondIndex(k);
	            pV2->NullifyInternalBondPointer(k);
	        }
	    }

        // erase the bond from the master array
        BondArrayInternal.erase(BondArrayInternal.begin() + trash[j-1]);

        // update all other bond pointers to the newly shifted index
        int iT = NumBond();
        for (int j=0; j<iT; j++) {

            BondDir nVoxBD, pVoxBD;
            switch (BondArrayInternal[j].GetBondAxis()){
            case AXIS_X: nVoxBD=BD_PX; pVoxBD=BD_NX; break;
            case AXIS_Y: nVoxBD=BD_PY; pVoxBD=BD_NY; break;
            case AXIS_Z: nVoxBD=BD_PZ; pVoxBD=BD_NZ; break;
            }

            pV1 = BondArrayInternal[j].GetpV1(); pV2 = BondArrayInternal[j].GetpV2();

            pV1->LinkInternalBond(j, nVoxBD);
	        pV2->LinkInternalBond(j, pVoxBD);

	        pV1->UpdateInternalBondPtrs();
	        pV2->UpdateInternalBondPtrs();
        }
...
void CVX_Sim::SetVoxData(void)
{
	...
	for (int i=0; i<NumVox(); i++) 
	{
		...
		VoxArray[i].minElasticMod = (pEnv->pObj->GetDeprecatedMinElasticMod()) ? pEnv->pObj->GetDeprecatedMinElasticMod() : pEnv->GetMinElasticMod();
		VoxArray[i].maxElasticMod = (pEnv->pObj->GetDeprecatedMaxElasticMod()) ? pEnv->pObj->GetDeprecatedMaxElasticMod() : pEnv->GetMaxElasticMod();
		VoxArray[i].maxStiffnessVariation = pEnv->GetMaxStiffnessVariation();
        ...
        // Insulation
        if(pEnv->pObj->GetUsingInsulation())
        {
			VoxArray[i].Insulation = pEnv->pObj->GetInsulation(i);
        }
        else
        {
            VoxArray[i].Insulation = false;
        }

        // ComputationalBoundary
        if(pEnv->pObj->GetUsingComputationalBoundary())
        {
			double thisDist2;
			double thisBoundary = pEnv->pObj->GetComputationalBoundary(i);
			VoxArray[i].ComputationalBoundary = thisBoundary;
            VoxArray[i].VoxWithinCompBoundary.clear();
			for (int s=0; s<NumVox(); s++)
			{
			    if (s == i) continue;
			    CVXS_Voxel* pV2 = &VoxArray[s];
				thisDist2 = (VoxArray[i].GetNominalPosition() - pV2->GetNominalPosition()).Length2();

				if (thisDist2 <= thisBoundary*thisBoundary)
				    VoxArray[i].VoxWithinCompBoundary.push_back(pV2);
			}
        }
        else
        {
            VoxArray[i].ComputationalBoundary = 0.0;
        }
		...
		int ioSize = 9;  // 1 currScale + 6 strains + 1 floor touch + 1 output
		if (pEnv->getUsingRegenerationModelInputBias())
		    ioSize += 1;
		for (int s=0; s < ioSize + pEnv->getNumHiddenRegenerationNeurons(); s++) {
		    VoxArray[i].RegenerationModelNeuronValues.push_back(0.0);
		}

		// stiffness model
		VoxArray[i].StiffnessModelNeuronValues.clear();
		ioSize = 3;  // 1 currStiffness + 1 strain (max) + 1 output
		if (pEnv->getUsingStiffnessModelInputBias())
		    ioSize += 1;
		for (int s=0; s < ioSize + pEnv->getNumHiddenStiffnessNeurons(); s++) {
		    VoxArray[i].StiffnessModelNeuronValues.push_back(0.0);
		}
...
//	for (int k=0; k<iT; k++) BondArrayCollision[k].ResetBond();
	...
	TimeOfLastDisintegrationUpdate = 0;
	TimeOfLastStiffnessModelUpdate = 0;

	MaxStressSoFar = 1e6;
	MaxPressureSoFar = 1e6;
	MinPressureSoFar = -1e6;

	NextVoxToRemove = pEnv->GetNumDisintegrationUpdates();
...
bool CVX_Sim::UpdateStats(std::string* pRetMessage) //updates simulation state (SS)
{
	...
	if ( pEnv->getUsingSavePassiveData() && (GetCurTime() < GetInitCmTime()) && (GetCurTime()+2*dt > GetInitCmTime()) )
	{
        ...
        for (int i=0; i<NumVox(); i++)
        {
            if (VoxArray[i].GetCurGroundPenetration().z > 0){ numTouchingGround += 1;}
            ...
            SS.TouchTrace.push_back(VoxArray[i].GetCurGroundPenetration().z);
...
//        std::cout << avgRoll << ", " << avgPitch << ", " << avgYaw << ", "<< dt << std::endl;
    ...
    if ( pEnv->GetDisintegrationUpdatesPerTempCycle() > 0 )
	{
        if (CurTime - TimeOfLastDisintegrationUpdate >= pEnv->GetTempPeriod() / pEnv->GetDisintegrationUpdatesPerTempCycle() )
        {
            TimeOfLastDisintegrationUpdate = CurTime;

            for (int i=0; i<pEnv->GetNumDisintegrationUpdates(); i++){
                if (pEnv->pObj->GetDisintegrationOrder(i) == NextVoxToRemove){
                    DetachVox(i);
                    break;
                }
            }
	...
	// Stiffness model updates
	if ( pEnv->GetStiffnessModelUpdatesPerTempCycle() > 0 )
	{
        if (CurTime - TimeOfLastStiffnessModelUpdate >= pEnv->GetTempPeriod() / pEnv->GetStiffnessModelUpdatesPerTempCycle() )
        {
            TimeOfLastStiffnessModelUpdate = CurTime;
            for (int i=0; i<iT; i++)
                VoxArray[i].UpdateStiffnessModelNetwork();

            for (int i=0; i<iT; i++)
                VoxArray[i].MaxStrainIntegral = 0.0;
        }
	}
	...
	if ( pEnv->GetRegenerationModelUpdatesPerTempCycle() > 0 )
	{
        if (CurTime - TimeOfLastRegenerationModelUpdate >= pEnv->GetTempPeriod() / pEnv->GetRegenerationModelUpdatesPerTempCycle() )
        {
            ...
            for (int i=0; i<iT; i++)
                VoxArray[i].UpdateRegenerationNetwork();

            for (int i=0; i<iT; i++){
                VoxArray[i].MaxStrainIntegral = 0.0;
                VoxArray[i].ProportionOfTimeTouchingGround = 0.0;
            }
        }
	}
			...
			for (int j=i+1; j<SurfVoxCount; j++){
				...
				if (Dist2 < FilterDist2 && !pV1->IsNearbyVox(SIndex2)){ //quick filter...
					...
					if (Dist2 < ActDist*ActDist){ //if within the threshold...
					    if (not StickyVoxels) {CreateColBond(SIndex1, SIndex2);} // create temporary bond
					    else{ // possibly create a permanent bond

                            // todo: fix sticky bonds

					        bool looseV1 = true;
					        bool looseV2 = true;
					        for (int k=0; k<6; k++){
					            if (pV1->GetInternalBondIndex(k) != -1) looseV1 = false;
					            if (pV2->GetInternalBondIndex(k) != -1) looseV2 = false;
					        }

                            if (looseV1 or looseV2){
                                Vec3D<> DiffVec = pV2->GetCurPos() - pV1->GetCurPos();
                                vfloat DiffAngle = atan2(DiffVec.y, DiffVec.x);  // todo visualize this
                                int bondIndex;

                                if ((DiffAngle > 3*PI/4) or (DiffAngle < -PI/4))
                                    bondIndex = CreateStickyBond(SIndex2, SIndex1);
                                else
                                    bondIndex = CreateStickyBond(SIndex1, SIndex2);

                                // todo: check z axis
//                                if (DiffVec.z > pV1->GetCurScale()*0.5)
//                                    bondIndex = CreateStickyBond(SIndex2, SIndex1);
//
//                                else if (DiffVec.z < pV1->GetCurScale()*0.5)
//                                    bondIndex = CreateStickyBond(SIndex1, SIndex2);

                                if (bondIndex == -1) CreateColBond(SIndex1, SIndex2);  // nonstick (temp) collision
                            }


                            // else CreateColBond(SIndex1, SIndex2);  // nonstick (temp) collision // todo?
					    }
                    }
				}
			}
	...
	if (StickyVoxels) UpdateAllBondPointers();
...
int CVX_Sim::GetNumTouchingFloor()
{
	...
	for (int i=0; i<LocNumVox; i++){
		if (VoxArray[i].GetCurGroundPenetration().z > 0) NumTouching++;
	}
	return NumTouching;
}
...
bool CVX_Sim::GetHasFallenOver()
{
	...
	for (int i=0; i<iT; i++)
	{
		if (VoxArray[i].GetMaterialIndex() == 7 && VoxArray[i].GetCurGroundPenetration().z > 0) return true;
	}
	return false;
}
