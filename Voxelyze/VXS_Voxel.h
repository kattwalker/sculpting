...
public:
	...
	Vec3D<> GetCurGroundPenetration(); //how far into the ground penetrating (penetration is positive, no penetration is zero)
    ...
    float StiffnessDirection;
    float TimeOfLastStiffnessDirectionUpdate;
    float lastInteroceptionIntegral;
    float currInteroceptionIntegral;
    ...
    // Stiffness model
    double oldStiffnessModelOutput;
    double currStiffnessModelOutput;
	void UpdateStiffnessModelNetwork();
	std::vector<double> StiffnessModelNeuronValues;
	std::vector<double> AbsStrainIntegralValues;
	double MaxStrainIntegral;
...
//	vfloat TimeOfLastRegenerationModelUpdate;
	double ProportionOfTimeTouchingGround;
	...
	bool disintegrated;
	bool NeighborJustDetached;

	bool Insulation;
	float ComputationalBoundary;
	std::vector<CVXS_Voxel*> VoxWithinCompBoundary;
	...
	vfloat GetDisintegrationStressIntegral(){return DisintegrationStressIntegral;};
	vfloat GetDisintegrationPressureIntegral(){return DisintegrationPressureIntegral;};
...
private:
	...
	vfloat DisintegrationStressIntegral;
	vfloat DisintegrationPressureIntegral;
