...
bool CVX_Bond::LinkVoxels(const int V1SIndIn, const int V2SIndIn)
{
    ...
    if (not p_Sim->StickyVoxels){
        ...
        // todo: sticky bonds
//        vfloat nVoxYaw = pVox1->GetYaw();
//	    vfloat pVoxYaw = pVox2->GetYaw();
//	    std::cout << "nVoxYaw = " << nVoxYaw << std::endl;

        Vec3D<> DiffVec = pVox2->GetCurPos() - pVox1->GetCurPos();  // todo visualize the angle fpr testing
        vfloat DiffAngle = atan2(DiffVec.y, DiffVec.x);
//        std::cout << "DiffAngle = " << DiffAngle << std::endl;

        if ((DiffAngle > -PI/4) and (DiffAngle < PI/4) and (pVox1->GetInternalBondIndex(0) == -1) and (pVox2->GetInternalBondIndex(1) == -1))
            ThisBondAxis = AXIS_X;
        else if ((DiffAngle > PI/4) and (DiffAngle < 3*PI/4) and (pVox1->GetInternalBondIndex(2) == -1) and (pVox2->GetInternalBondIndex(3) == -1))
            ThisBondAxis = AXIS_Y;
        else if ((DiffAngle > 3*PI/4) or (DiffAngle < -5*PI/4) and (pVox1->GetInternalBondIndex(0) == -1) and (pVox2->GetInternalBondIndex(1) == -1))
            ThisBondAxis = AXIS_X;
        else if ((DiffAngle > -5*PI/4) and (DiffAngle < -PI/4) and (pVox1->GetInternalBondIndex(2) == -1) and (pVox2->GetInternalBondIndex(3) == -1))
            ThisBondAxis = AXIS_Y;

        else if ((fabs(DiffVec.z) > pVox1->GetCurScale()*0.5) and (pVox1->GetInternalBondIndex(4) == -1) and (pVox2->GetInternalBondIndex(5) == -1))
            ThisBondAxis = AXIS_Z;
